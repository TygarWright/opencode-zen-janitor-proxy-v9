...
def model_for(model, provider):
    ...
    if low in MODELS:
        return MODELS[low][0 if provider == "opencode" else 1], low
...
def upstream_headers(provider, key, stream):
    ...
    if provider == "opencode": h["User-Agent"] = os.getenv("OPENCODE_USER_AGENT", "OpenCode/1.2.31")
    return h
...
async def get_buffered(provider, payload):
    ...
    for slot, key in enumerate(keys(provider), 1):
        try:
            ...
            if 200 <= status < 300:
                try:
                    return json.loads(text), status, slot
                except Exception:
                    return None, 502, slot
            last = (status, text[:500])
            r.release()
            if status not in RETRY:
                return None, status, slot
        except Exception as e:
            last = (502, str(e)[:300])
...
async def chat(req):
    if not auth(req):
        return error(401, "Invalid proxy API key")
    try:
        body = await req.json()
    except Exception:
        return error(400, "Invalid JSON")
    if not isinstance(body, dict):
        return error(400, "JSON object required")
    ...
    for index, provider in enumerate(order):
        ...
        if status not in RETRY:
            return error(status, "Upstream request failed")
...
async def health(req):
    return web.json_response({
        "ok": True,
        "service": "janitorai-multiproxy-v9",
        "providers": {"opencode": bool(keys("opencode")), "bitdeer": bool(keys("bitdeer"))},
    })
